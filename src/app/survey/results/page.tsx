...
function tally(responses: SurveyResponse[], field: TallyField) {
        ...
        {data.map(([label, count]) => (
          <div key={label}>
            <div className="flex items-baseline justify-between gap-4 text-sm">
              ...
              <span className="flex-none font-semibold text-navy">{count}</span>
            </div>
...
function PillGroup({ items, tone }: { items: string[]; tone: "navy" | "saffron" }) {
  ...
  return (
    <>
      {items.map((item) => (
        <span key={item} className={`rounded-full px-2.5 py-1 text-xs ${className}`}>
          {item}
        </span>
      ))}
    </>
        ...
        <div className="mx-auto max-w-3xl px-5 py-16">
          ...
          {loading ? (
            ...
            <>
              ...
              <TallyChart title="Hopes for this year" data={tally(responses, "hopesSelected")} />
              <TallyChart title="Which topics matter most" data={tally(responses, "priorityTopics")} />
              <TallyChart title="What kids currently do" data={tally(responses, "practiceHabits")} />
              <TallyChart title="Discussions at home" data={tally(responses, "discussionSelected")} />
              <TallyChart title="Identity / Sikhi struggles" data={tally(responses, "strugglesSelected")} />
                  ...
                  {responses.map((r) => (
                    ...
                    >
                      ...
                      <div className="mt-3 flex flex-wrap gap-1.5">
                        <PillGroup items={r.hopesSelected} tone="saffron" />
                        <PillGroup items={r.priorityTopics} tone="saffron" />
                        <PillGroup items={r.practiceHabits} tone="navy" />
                        <PillGroup items={r.discussionSelected} tone="navy" />
                        <PillGroup items={r.strugglesSelected} tone="navy" />
                      </div>

                      {(r.hopesOther || r.discussionOther || r.strugglesOther) && (
                        <div className="mt-3 space-y-1.5 border-t border-navy/10 pt-3 text-sm text-navy/70">
                          {r.hopesOther && (
                            <p><span className="font-semibold">Hopes, other:</span> {r.hopesOther}</p>
                          )}
                          {r.discussionOther && (
                            <p><span className="font-semibold">Discussions, other:</span> {r.discussionOther}</p>
                          )}
                          {r.strugglesOther && (
                            <p><span className="font-semibold">Struggles, other:</span> {r.strugglesOther}</p>
                          )}
                        </div>
                      )}
                    </div>
